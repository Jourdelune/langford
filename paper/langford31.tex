...
\author{Claude Opus 5\thanks{Anthropic. This paper, the algorithm it describes,
its CUDA implementation, the distributed orchestration, the verification chain
and the computation itself were produced by the author, an AI system. The human
collaborator set the objectives, provided hardware access and arbitrated
spending. See §\ref{sec:authorship}.}}
...
A \emph{Langford pairing} of order $n$ places each of the integers
...
\texttt{A014552}~\cite{oeis}; enumeration by backtracking reaches $n=24$, and the
algebraic method described below carried the sequence to $n=27$ and $n=28$ in
2015~\cite{abl}. There it stopped for eleven years.
...
\subsection{Why \texorpdfstring{$V(n) = 2L(2,n)$}{V(n) = 2L(2,n)}}

The two quantities count different objects. Equation~\eqref{eq:godfrey} produces
$V(n)$, the number of \emph{arrangements}: sequences as written, left to right.
The conventional $L(2,n)$ of \texttt{A014552} counts pairings \emph{up to
reversal}, treating a sequence and its mirror image as one object. The factor of
two between them is exact, and for a reason worth stating.

\begin{proposition}
No Langford pairing is its own mirror image.
\end{proposition}

\begin{proof}
Suppose an arrangement is invariant under the reflection $p \mapsto 2n+1-p$.
The pair carrying value $k$ occupies positions $i$ and $i+k+1$; its image is
$\{2n-i-k,\; 2n+1-i\}$. Invariance forces
$\{2n-i-k,\,2n+1-i\} = \{i,\,i+k+1\}$, hence $2i = 2n-k$. For odd $k$ this has
no integer solution, and since the values run over $1,\dots,n$ with $n \ge 3$ an
odd $k$ always occurs.
\end{proof}

The reflection is therefore a fixed-point-free involution: it pairs arrangements
exactly two by two, and $V(n)$ is even. The implementation turns this into a
test, requiring divisibility of the raw total by $2^{2n+1}$ rather than
$2^{2n}$. The extra bit is precisely this one, and it converts an odd $V$ ---
which the final shift would otherwise truncate in silence --- into an error
...
\emph{Kasteleyn's method does not apply.} A Pfaffian orientation would give
$2^n n^3$ in place of $4^n$, and the record would fall in minutes. We refute its
...
\paragraph{2. Arithmetic self-test.} By $A_i \equiv i \pmod 2$, the
$\lfloor (n+1)/2 \rfloor$ even gaps each contribute a factor of two, so the raw
total must be divisible by $2^{2n+1} = 2^{63}$. A single corrupted slice fails
the test. \emph{Catches} a partial sum altered in transit or in memory. This
check runs before any rented machine is released.
